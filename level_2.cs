...
{
    public class Level_2 : RC_GameStateParent
    {
        Texture2D texBG;
        Texture2D texPlat;
        Texture2D texPlayer;
        ...
        public override void LoadContent()
        {
            ...
            texSpike = Content.Load<Texture2D>("Images/spike");
           
            door = new Sprite3(true, texDoor, 700, 400 - texDoor.Height);
            door1 = new Sprite3(true, texDoor1, 700, 400 - texDoor1.Height);
            ...
            winSound = Content.Load<SoundEffect>("music/mixkit-retro-game-notification-212");
            loseSound = Content.Load<SoundEffect>("music/mixkit-retro-arcade-game-over-470");
            ...
            for (int i = 0; i < 2; i++)
            {
                block = new Sprite3(true, texPlat, 100 + i * texPlat.Width, 150);
                platfrom_1.addSpriteReuse(block);
            }
            for (int i = 0; i < 2; i++)
            {
                block = new Sprite3(true, texPlat, 800 + i * texPlat.Width, 150);
                platfrom_2.addSpriteReuse(block);
            }
            for (int i = 0; i < 8; i++)
            {
                block = new Sprite3(true, texPlat, 250 + i * texPlat.Width, 400);
            ...
            for (int i = 0; i < 3; i++)
            {
                if (i == 1)
                {
                    candy = new Sprite3(true, texCandy, 800, 150 - texCandy.Height);
                }
                else
                    candy = new Sprite3(true, texCandy, 100+ i*texCandy.Width, 150 - texCandy.Height);
            ...
            for (int i = 0; i < 2; i++)
            {
                if (i == 1)
                {
                    spike = new Sprite3(true, texSpike, 850, 150 - texSpike.Height);
                }
                else
                    spike = new Sprite3(true, texSpike, 300, 400 - texSpike.Height);
                spikeList.addSpriteReuse(spike);
            }
            for (int i = 0; i < 3; i++)
            {
                candy = new Sprite3(true, texCandy, 400 + i * (texCandy.Width + 50), 400 - texCandy.Height);
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            {
                Sprite3 s = spikeList.getSprite(i);
                if (player.collision(s))
                {
